...
class PlaceUnavailabilityControllerIT extends AbstractIntegrationTest {
    ...
    void getPlaceUnavailability_byOwner_returns200WithList() throws Exception {
        ...
        mockMvc.perform(get(ENDPOINT, 9000000011L).with(user(owner)))
    ...
    void getPlaceUnavailability_byRegularUser_returns200WithList() throws Exception {
        var regularUser = createCustomUserDetails(99999999998L, "test.user@gmail.com", "Test", "User",
                "{bcrypt}Test@12345678941", Set.of(Role.builder().name(RoleEnum.USER).build()));

        mockMvc.perform(get(ENDPOINT, 9000000011L).with(user(regularUser)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(9000002001L))
                .andExpect(jsonPath("$[0].placeId").value(9000000011L))
                .andExpect(jsonPath("$[0].reason").value("OWNER_BLOCKED"));
    ...
    void getPlaceUnavailability_placeNotFound_returns404() throws Exception {
        ...
        mockMvc.perform(get(ENDPOINT, 9999999999L).with(user(regularUser)))
